...
// Penalty for IRES <-> rest pairs. Uses the same boundary logic as pair_blocked
// but returns log(lambda) instead of blocking: soft penalty, not hard block.
// When lambda=1 -> penalty=0 (no effect). When lambda->0 -> penalty->-inf (≈hard block).
int g_pair_block_p1 = -1;
int g_pair_block_p2 = -1;
bool g_pair_block_enabled = false;
ScoreType g_cross_penalty = 0.0;

void init_pair_block_params(int ires_end_val, IndexType total_seq_length, double lambda) {
    if (ires_end_val <= 0 || lambda >= 1.0 - 1e-10) {
    ...
    g_cross_penalty = std::log(lambda);
}

// Returns log(lambda) if (i,j) crosses IRES <-> rest boundary, 0 otherwise. 0-based positions.
inline ScoreType cross_penalty(IndexType i, IndexType j) {
    if (!g_pair_block_enabled) return 0.0;
    if (i > j) std::swap(i, j);
    int i1 = static_cast<int>(i) + 1;
    int j1 = static_cast<int>(j) + 1;
    if ((i1 <= g_pair_block_p1) && (j1 >= g_pair_block_p2))
        return g_cross_penalty;
    return 0.0;
}
    ...
    for (const auto& j_redge_ptr : dfa.right_edges[j_node]) {
        ...
        for (const auto& j4_node : dfa.nodes[j + 4]) {
            ...
            for (const auto& jnext_node_nuc_param : jnext_list) { // Item Type: tuple<NodeType, NucType, Parameter*>
                ...
                ScoreType cp_jjnext = cross_penalty(j, jnext);
                ...
                if (hairpin_length == 5 or hairpin_length == 6 or hairpin_length == 8){
                    for(auto & seq_score_weight : hairpin_seq_score_cai[j_node][jnext_node][NTP(j_nuc, jnext_nuc)]){
                            ...
                            ScoreType score_sp_hairpin =  get<1>(seq_score_weight) / kT + cp_jjnext;
                            Parameters params;
                ...
                for (const auto &j1_redge_ptr : dfa.right_edges[j1_node]) {
                    ...
                    for (const auto& jnext_1_node2ledges : dfa.auxiliary_left_edges[jnext_node]){
                        ...
                        for (const auto& jnext_ledge_ptr : jnext_1_node2ledges.second){
                            ...
                            ScoreType score_hairpin = - func4(j, jnext, j_nuc, j1_nuc, jnext_1_nuc, jnext_nuc, tetra_hex_tri) / kT + cp_jjnext;
        ...
        for (auto &pair_elem: bestH[j_node][i_node]) {
            ...
            for (const auto &j1_node2redges: dfa.auxiliary_right_edges[j_node]) { // Item Type: unordered_map<NodeType, vector<EdgeType*>>
                ...
                for (const auto &jnext_node_nuc_param: jnext_list) { // Item Type: tuple<NodeType, NucType, Parameter*>
                    ...
                    ScoreType cp_ijnext = cross_penalty(i, jnext);
                    ...
                    if (hairpin_length == 5 or hairpin_length == 6 or hairpin_length == 8){
                        for(auto & seq_score_weight : hairpin_seq_score_cai[i_node][jnext_node][NTP(i_nuc, jnext_nuc)]){
                                ...
                                assert(!sp_hairpin_path.empty());
                                ScoreType score_sp_hairpin =  get<1>(seq_score_weight) / kT + cp_ijnext;
                                Parameters params;
                                for (const auto& edge_ptr : sp_hairpin_path) {
            ...
            if (!jnext_list.empty()) {
                ...
                for (const auto &jnext_node_nuc_param: jnext_list) { // Item Type: tuple<NodeType, NucType, Parameter*>
                    ...
                    for (const auto &jnext_redge_ptr: dfa.right_edges[jnext_node]) {
                        ...
                        ScoreType cp_multi = cross_penalty(i_node.first, jnext);
                        NucPairType pair_nuc_i_jnext = NTP(i_nuc, jnext_nuc);

                        update_state<phase>(bestMulti[jnext1_node][i_node][pair_nuc_i_jnext], state_Multi, cp_multi, params, state_Multi.pre_node);
                ...
                for (const auto &j_redge_ptr: dfa.right_edges[j_node]) {
                    ...
                    for (const auto &i_ledge_ptr: dfa.left_edges[i_node]) {
                        ...
                        if (_allowed_pairs[i_1_nuc][j_nuc]) {
                            ScoreType score_stacking = stacking_score[outer_pair - 1][pair_nuc - 1] / kT + cross_penalty(i_1_node.first, j);
                    ...
                    for (const auto &i_ledge_ptr: dfa.left_edges[i_node]) {
                        ...
                        for (const auto &q_node_nuc_param: q_list) { // Item Type: tuple<NodeType, NucType, Parameter*>
                            ...
                            for (const auto &q1_node2redges: dfa.auxiliary_right_edges[q_node]) {
                                ...
                                ScoreType score_bulge = bulge_score[outer_pair - 1][pair_nuc - 1][q - j - 1] / kT + cross_penalty(i_1_node.first, q);
                    ...
                    for (const auto &i_1_node2ledges: dfa.auxiliary_left_edges[i_node]) {
                        ...
                        for (const auto &p_node_nuc_param: p_list) { // Item Type: tuple<NodeType, NucType, Parameter*>
                            ...
                            for (const auto &p_ledge_ptr: dfa.left_edges[p_node]) {
                                ...
                                ScoreType score_bulge = bulge_score[outer_pair - 1][pair_nuc - 1][i - p - 1] / kT + cross_penalty(p_1_node.first, j);
                        ...
                        for (IndexType p = i - 1;
                            ...
                            for (const auto &p_node: p_node_list) {
                                for (const auto &p_redge_ptr: dfa.right_edges[p_node]) {
                                    ...
                                    IndexType p_dfa_plus1 = p_dfa + 1;
                                    IndexType i_dfa_minus1 = i_dfa - 1;
                                    if (p == i - 1 and p_nuc != i_1_nuc) continue;
                                    else if (p == i - 2 and p1_num != i_1_num) continue;
                                    else if (p == i - 3 and p1_num != i_1_num and
                                             p_dfa_plus1 >= 0 && p_dfa_plus1 < (int)dfa.nodes.size() &&
                                             i_dfa_minus1 >= 0 && i_dfa_minus1 < (int)dfa.nodes.size() &&
                                             dfa.nodes[p_dfa_plus1].size() == dfa.nodes[i_dfa_minus1].size())
                                        continue;
                                    ...
                                    for (const auto &p_ledge_ptr: dfa.left_edges[p_node]) {
                                        ...
                                        for (const auto &q_node_nuc_param: q_list) { // Item Type: tuple<NodeType, NucType, Parameter*>
                                            ...
                                            for (const auto &q1_node2redges: dfa.auxiliary_right_edges[q_node]) {
                                                ...
                                                ScoreType cp_internal = cross_penalty(p_1_node.first, q);
                                                            ...
                                                            for (const auto &q_ledge_ptr: q_1_node2ledges.second) {
                                                                ...
                                                                ScoreType score_internal =
                                                                        -func5(p - 1, q, i, j - 1, p_1_nuc,
                                                                                        ...
                                                                                        i_nuc, j_1_nuc, j_nuc) / kT + cp_internal;
        ...
        for (IndexType p = i-1; p >= max(i - SINGLE_MAX_LEN, 0); --p){
            ...
            for (const auto &p_node : p_node_list){
                for (const auto &p_redge_ptr : dfa.right_edges[p_node]){
                    ...
                    IndexType p_dfa_plus1 = p_dfa + 1;
                    if(p == i - 2 and p_dfa_plus1 < (int)dfa.nodes.size() && i_dfa < (int)dfa.nodes.size() && dfa.nodes[p_dfa_plus1].size() == dfa.nodes[i_dfa].size() and p1_node.second != i_node.second) continue;
                    ...
                    for (const auto& q_node_nuc_param : q_list){
                        ...
                        ScoreType cp_m2 = cross_penalty(p_node.first, q_node.first);
                        for (const auto& q_redge_ptr : dfa.right_edges[q_node]){
                            ...
                            update_state<phase>(bestMulti[q1_node][p_node][outer_pair], state_M2, cp_m2, params, j_node);
                            break;
                        }
                    }
                }
        ...
        while (!q_list.empty()){
            ...
            for (const auto& q_node_nuc_param: q_list){
                ...
                for(const auto& q1_node2redges : dfa.auxiliary_right_edges[q_node]){
                    ...
                    if(q < (int)dfa.nodes.size() && q+1 < (int)dfa.nodes.size() && dfa.nodes[q].size() == 1 and dfa.nodes[q+1].size() == 2 and q_nuc != q_nuc_) continue;
...
void Optimizer::optimize(
    ...
    init_pair_block_params(ires_end, seq_length, ires_orf_lambda);
    ...
    int orf_len = seq_length - fixed_prefix_len;
    if (ires_end > 0 && ires_orf_lambda < 1.0 - 1e-10) {
        cout << "Cross-pair penalty: lambda=" << ires_orf_lambda
             << " (energy penalty=" << std::setprecision(2) << (-kT * std::log(ires_orf_lambda))
             << " kT) IRES=[0," << ires_end - 1 << "] vs rest=["
             << ires_end << "," << seq_length - 1 << "]" << endl;
    }
    ...
    for(int k=1; k<=num_epochs; ++k){
        ...
        if(end_state.inside > best_obj){
            ...
            best_nuc_seq.first = full_seq;
            cout << "Iteration [" << setw(width) << setfill(' ') << k << "]: obj: "
                 << std::setprecision(4) << fixed << -end_state.inside << endl;
        } else {
            cout << "Iteration [" << setw(width) << setfill(' ') << k << "]: obj: "
                 << std::setprecision(4) << fixed << -end_state.inside << endl;
        }
...
Optimizer::Optimizer(int beam_size_, int num_epochs_, double learning_rate_, double epsilon_, std::string init_solution_, bool is_verbose_, unsigned int rand_seed_, std::string ires_, int fixed_prefix_len_, double ires_orf_lambda_, double cross_pair_prob_threshold_, int max_cross_pairs_, std::string suffix_, int ires_end_)
        : beam_size(beam_size_), num_epochs(num_epochs_), learning_rate(learning_rate_), epsilon(epsilon_),
          init_solution(init_solution_), is_verbose(is_verbose_), rand_seed(rand_seed_), ires(ires_), fixed_prefix_len(fixed_prefix_len_), ires_orf_lambda(ires_orf_lambda_), cross_pair_prob_threshold(cross_pair_prob_threshold_), max_cross_pairs(max_cross_pairs_), suffix(suffix_), fixed_suffix_len(suffix_.length()), ires_end(ires_end_) {
        func1();
}
