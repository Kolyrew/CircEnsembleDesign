...
public:
    ...
    Optimizer(int beam_size_, int num_epochs_, double learning_rate_, double epsilon_, std::string init_solution_, bool is_verbose_, unsigned int rand_seed_, std::string ires_, int fixed_prefix_len_ = 0, double ires_orf_lambda_ = 1.0, double cross_pair_prob_threshold_ = 0.01, int max_cross_pairs_ = 5, std::string suffix_ = "", int ires_end_ = 0);
...
private:
    ...
    // Helper methods for IRES-ORF penalty (circular RNA model)
    // 
    // In circular RNA: [IRES] -> [ORF] -> wraps back to [IRES]
    // The END of ORF is physically adjacent to the BEGINNING of IRES.
    // Structures forming at the end of ORF (close to IRES in the circle)
    // are penalized to discourage IRES↔ORF interactions.
    //
    // The boundary zone is the last SINGLE_MAX_LEN nucleotides of ORF.
    // Any base pair involving a position in this zone receives the penalty.
    
    // Check if DFA position is in the boundary zone (end of ORF, close to IRES in circle)
    inline bool is_boundary_zone(IndexType i_dfa) const {
        if (fixed_prefix_len == 0) return false;  // No IRES, no boundary
        IndexType orf_length = seq_length - fixed_prefix_len;
        IndexType zone_start = orf_length - SINGLE_MAX_LEN;
        if (zone_start < 0) zone_start = 0;  // ORF shorter than boundary zone
        return i_dfa >= zone_start && i_dfa < orf_length;
    }
    
    // Get the log penalty for boundary zone base pairing
    ...
    // Apply IRES-ORF boundary penalty if position is in boundary zone
    // i_dfa is the DFA position of a nucleotide in a base pair
    // Penalizes structures at the end of ORF that could interact with IRES in circular RNA
    inline ScoreType apply_boundary_penalty(IndexType i_dfa) const {
        if (is_boundary_zone(i_dfa)) {
            return get_ires_orf_penalty();
        }
        return 0.0;
    }
    
    // Apply penalty checking both positions of a base pair
    // i_abs and j_abs are absolute positions (including IRES offset)
    // DISABLED: penalty is turned off; cross-pair control is done via
    // probabilistic detection + rejection, not via DP energy penalty
    ...
    // ==================== IRES DFA Extension ====================
    // Virtual DFA structures for IRES region to enable IRES-ORF base pairing
    // IRES positions use negative DFA indices: [-fixed_prefix_len, 0)
    // which correspond to absolute positions [0, fixed_prefix_len)
    
    // IRES nodes: one node at each IRES position (deterministic)
    // Key: absolute position [0, fixed_prefix_len), Value: NodeType
    std::vector<NodeType> ires_nodes;
    
    // IRES edges: deterministic transitions (fixed nucleotides from ires string)
    std::vector<EdgeType> ires_edges;
    std::vector<Parameter> ires_parameters;  // Fixed parameters (weight=0, no optimization)
    
    // Maps for IRES edge access
    std::unordered_map<NodeType, std::vector<EdgeType*>, hash_pair> ires_right_edges;
    std::unordered_map<NodeType, std::vector<EdgeType*>, hash_pair> ires_left_edges;
    
    // Initialize IRES DFA structures
    void init_ires_dfa();
    
    // Get nucleotide at IRES position (absolute position must be < fixed_prefix_len)
    inline NucType get_ires_nuc(IndexType abs_pos) const {
        if (abs_pos >= 0 && abs_pos < fixed_prefix_len) {
            return GET_ACGU_NUC(ires[abs_pos]);
        }
        return 0;  // Invalid
    }
    
    // Check if position is valid for pairing (either in IRES or ORF)
    inline bool is_valid_pair_position(IndexType abs_pos) const {
        return abs_pos >= 0 && abs_pos < seq_length;
    }
    
    // Get IRES node at absolute position
    inline NodeType get_ires_node(IndexType abs_pos) const {
        if (abs_pos >= 0 && abs_pos < fixed_prefix_len) {
            return ires_nodes[abs_pos];
        }
        return NodeType(-1, -1);  // Invalid
    }
    
    // Convert absolute position to extended DFA position
    // For IRES: abs_pos -> abs_pos (stays the same, but handled specially)
    // For ORF: abs_pos -> abs_pos - fixed_prefix_len (DFA position)
    inline IndexType abs_to_dfa(IndexType abs_pos) const {
        return abs_pos - fixed_prefix_len;
    }
    
    // Convert extended DFA position to absolute position
    inline IndexType dfa_to_abs(IndexType dfa_pos) const {
        return dfa_pos + fixed_prefix_len;
    }
    // ==================== End IRES DFA Extension ====================
